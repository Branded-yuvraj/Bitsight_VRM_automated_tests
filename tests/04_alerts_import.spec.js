...
test.describe.serial('Type 1 Token CM - Alerts Import and Incident Tests', () => {
    ...
    test('TC-04 Type 1: No Incident Created for Critical Severity alerts when critical_alert_inc flag is disabled', async ({ page }) => {
        test.setTimeout(1_800_000); // 30 minutes for full import + reconciliation
        ...
        // -------------------------------------------------------------------------
        // Pre-Step 2: Configure Application Properties - Disable Critical flag
        // -------------------------------------------------------------------------
        console.log('\n=== Pre-Step 2: Configuring Application Properties (Disabling Critical Alert Trigger) ===');
        await filterBitsightModules(page);
        await page.getByRole('link', { name: /^Application Configuration \d+ of \d+$/ }).click();
        ...
        // 3. Read back saved configurations dynamically and confirm the flag is off
        const savedConfig = {
            insCompany: await configFrame.locator('#ins_company_y').isChecked(),
            markComp: await configFrame.locator('#mark_comp_y').isChecked(),
            criticalAlertInc: await configFrame.locator('#critcal_alert_inc_y').isChecked(),
        ...
        console.log('Waiting 30 seconds for post-import transform scripts to run...');
        await page.waitForTimeout(30_000);
    ...
    test('TC-05 Type 1: Incident Creation for Warn Severity alerts based on Application Configuration', async ({ page }) => {
        test.setTimeout(1_800_000); // 30 minutes for full import + reconciliation
    ...
    test('TC-06 Type 1: No Incident Created for Warn Severity alerts when inc_warn_alert flag is disabled', async ({ page }) => {
        test.setTimeout(1_800_000); // 30 minutes for full import + reconciliation
        ...
        // -------------------------------------------------------------------------
        // Pre-Step 2: Configure Application Properties - Disable Warn flag
        // -------------------------------------------------------------------------
        console.log('\n=== Pre-Step 2: Configuring Application Properties (Disabling Warn Alert Trigger) ===');
        await filterBitsightModules(page);
        await page.getByRole('link', { name: /^Application Configuration \d+ of \d+$/ }).click();
    ...
    test('TC-07 Type 1: Incident Creation when both Critical and Warn Severity alert triggers are enabled', async ({ page }) => {
        test.setTimeout(1_800_000); // 30 minutes for full import + reconciliation
    ...
    test('TC-08 Type 1: No Incident Created when both Critical and Warn Severity alert triggers are disabled', async ({ page }) => {
        test.setTimeout(1_800_000); // 30 minutes for full import + reconciliation
        ...
        // -------------------------------------------------------------------------
        // Pre-Step 2: Configure Application Properties - Disable Both Critical and Warn flags
        // -------------------------------------------------------------------------
        console.log('\n=== Pre-Step 2: Configuring Application Properties (Disabling Both Alert Triggers) ===');
        await filterBitsightModules(page);
        await page.getByRole('link', { name: /^Application Configuration \d+ of \d+$/ }).click();
        ...
        // 3. Read back saved configurations dynamically and confirm both flags are off
        const savedConfig = {
            insCompany: await configFrame.locator('#ins_company_y').isChecked(),
            markComp: await configFrame.locator('#mark_comp_y').isChecked(),
            criticalAlertInc: await configFrame.locator('#critcal_alert_inc_y').isChecked(),
...
test.describe.serial('Type 3 Token CM_VRM - Alerts Import and Incident Tests', () => {
    ...
    test('TC-03 Type 3: Incident Creation for Critical Severity alerts based on Application Configuration', async ({ page }) => {
        ...
        await configFrame.locator('#ins_company_y').check();
        await configFrame.locator('#mark_comp_y').check();
        await configFrame.locator('#maxpropertyinc').fill('50');
        await configFrame.locator('#inc_score_n').check();
        await configFrame.locator('#critcal_alert_inc_y').check();
        await configFrame.locator('#inc_warn_alert_n').check();
        await configFrame.locator('#assign-incident').selectOption('user');
    ...
    test('TC-04 Type 3: No Incident Created for Critical Severity alerts when critical_alert_inc flag is disabled', async ({ page }) => {
        test.setTimeout(1_800_000); // 30 minutes for full import + reconciliation
    ...
    test('TC-05 Type 3: Incident Creation for Warn Severity alerts based on Application Configuration', async ({ page }) => {
        test.setTimeout(1_800_000); // 30 minutes for full import + reconciliation
    ...
    test('TC-06 Type 3: No Incident Created for Warn Severity alerts when inc_warn_alert flag is disabled', async ({ page }) => {
        test.setTimeout(1_800_000); // 30 minutes for full import + reconciliation
    ...
    test('TC-07 Type 3: Incident Creation when both Critical and Warn Severity alert triggers are enabled', async ({ page }) => {
        test.setTimeout(1_800_000); // 30 minutes for full import + reconciliation
    ...
    test('TC-08 Type 3: No Incident Created when both Critical and Warn Severity alert triggers are disabled', async ({ page }) => {
        test.setTimeout(1_800_000); // 30 minutes for full import + reconciliation
    ...
    test('TC-09 Type 3: Incident Creation on Bitsight Security Rating Score Drop', async ({ page }) => {
        ...
        // Step 1: Ensure Application Properties are configured (UI)
        ...
        await configFrame.locator('#mark_comp_y').check();
        await configFrame.locator('#inc_score_y').check();
        await configFrame.locator('#incscoredrop').fill(TARGET_SCORE_DROP);
        await configFrame.locator('#assign-incident').selectOption('user');
        ...
        const savedScoreDrop = await configFrame.locator('#incscoredrop').inputValue();
        expect(savedScoreDrop).toBe(TARGET_SCORE_DROP);

        // Step 2: Fetch a target company record from core_company
        ...
        try {
            ...
            // Step 6: Roll Back / Revert the Security Rating
    ...
    test('TC-10 Type 3: No Incident Created on Score Drop when inc_score flag is disabled', async ({ page }) => {
        test.setTimeout(180_000); // 3 minutes
        ...
        // Validate flag is confirmed disabled
        const isChecked = await configFrame.locator('#inc_score_y').isChecked();
        expect(isChecked, 'Score drop incident trigger should be disabled').toBeFalsy();

        // Step 2: Fetch Target Company Record
        console.log('\n=== Step 2: Fetching Target Company Record ===');
        const companies = await serviceNowClient.getTableRecords('core_company', {
            sysparm_query: 'x_bisit_vrm_bitsight_vendor_guidISNOTEMPTY^x_bisit_vrm_security_ratingISNOTEMPTY',
            sysparm_fields: 'sys_id,name,x_bisit_vrm_security_rating',
